...
import { heatShimmerVertexShader, heatShimmerFragmentShader } from "../../shaders/heatShimmer";
...
class AtmosphericParticlePool {
  ...
  spawn(type: ParticleData["type"], position: THREE.Vector3, velocity: THREE.Vector3, lifetime: number, size: number, color: THREE.Color): ParticleData | null {
  ...
  update(deltaTime: number, windVector: THREE.Vector3, bounds: { min: THREE.Vector3, max: THREE.Vector3 }) {
    let writeIndex = 0;
    ...
    for (let i = 0; i < this.activeCount; i++) {
      ...
      if (particle.lifetime > 0) {
        ...
        // Apply wind
        if (particle.type === "dust" || particle.type === "pollen" || particle.type === "ash") {
          particle.position.add(windVector.clone().multiplyScalar(deltaTime * 2));
        } else if (particle.type === "fog") {
          particle.position.add(windVector.clone().multiplyScalar(deltaTime * 0.5));
        }
        ...
        if ((particle.type === "rain" || particle.type === "acid") && particle.position.y < 0) {
          particle.position.y = bounds.max.y;
          particle.velocity.y = 0;
        }
        
        // Update opacity based on lifetime
        particle.opacity = Math.min(1, particle.lifetime / particle.maxLifetime);
...
export function AtmosphericEffects({ planetName, position = [0, 0, 0], flashlightOn = false }: AtmosphericEffectsProps) {
  ...
  useEffect(() => {
    if (atmosphere.fogDensity > 0) {
      ...
      if (flashlightOn) {
        scene.fog = new THREE.FogExp2(atmosphere.fogColor, atmosphere.fogDensity * 0.5);
      }
    } else {
      scene.fog = null;
    }
  ...
  useFrame((state, delta) => {
    if (!enableParticles || !particlePoolRef.current || !particlesRef.current) return;
    ...
    // Spawn new particles
    if (currentTime - lastSpawnTime.current > spawnInterval && atmosphere.particleTypes.length > 0) {
      lastSpawnTime.current = currentTime;
      
      const particleType = atmosphere.particleTypes[Math.floor(Math.random() * atmosphere.particleTypes.length)] as ParticleData["type"];
        ...
        switch (particleType) {
          ...
          case "fog":
            ...
            spawnPos = new THREE.Vector3(
              ...
              getHeightAt(camera.position.x, camera.position.z) + Math.random() * 5,
    ...
    const bounds = {
      min: new THREE.Vector3(camera.position.x - 100, -10, camera.position.z - 100),
      max: new THREE.Vector3(camera.position.x + 100, 40, camera.position.z + 100)
    };
