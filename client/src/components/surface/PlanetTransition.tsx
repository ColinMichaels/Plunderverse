...
export function PlanetTransitionOverlay(props: PlanetTransitionProps) {
  ...
  console.log("[PlanetTransitionOverlay] Render state:", {
    running,
    fadeOpacity,
  ...
  useEffect(() => {
    ...
    return () => {
      console.log(
        "[PlanetTransitionOverlay] Component unmounting, resetting fade",
      );
      setFadeOpacity(0);
    };
  }, []);
    ...
    >
      ...
      <div style={{ position: "absolute", inset: 0, background: "black" }}>
        <Canvas
          ...
          dpr={[1, 2]}
          camera={{
            position: initialCameraPos as [number, number, number],
            fov: 50,
            near: 0.1,
            far: 5000,
          }}
        >
          <PlanetTransitionScene
            ...
            onComplete={() => {
              console.log(
                "[PlanetTransitionOverlay] Scene animation complete, setting running to false",
              );
              setRunning(false);
              console.log("[PlanetTransitionOverlay] Calling props.onComplete");
              props.onComplete?.();
              console.log(
                "[PlanetTransitionOverlay] onComplete callback finished",
              );
            }}
  ...
  useFrame((state) => {
    ...
    if (lightRef.current) {
      ...
      lightRef.current.intensity =
        intensity * 3 * (Math.sin(state.clock.elapsedTime * 5) * 0.2 + 0.8);
    }
  });
...
function SmokeTrail({
  ...
  const emitOne = () => {
    ...
    const offset = right
      .clone()
      .multiplyScalar(Math.cos(a) * ringR)
...
function PlanetTransitionScene({
  ...
  const currentPlanet = planetData
    ? {
        textureUrl:
          planetData.texture ||
          `/textures/planets/2k_${planetName?.toLowerCase()}_daymap.jpg`,
  ...
  const textureUrl =
    currentPlanet?.textureUrl ?? "/textures/planets/2k_earth_daymap.jpg";
  ...
  const [starfieldOpacity, setStarfieldOpacity] = useState(
    direction === "takeoff" ? 0 : 1,
  );
  ...
  const hemi = useMemo(
    () => new THREE.HemisphereLight("#8fb8ff", "#1a1a1a", 0.35),
    [],
  );
  useEffect(() => {
    (cam as any).add?.(hemi);
  ...
  useEffect(() => {
    ...
    animationTimeoutRef.current = setTimeout(
      () => {
        console.warn(
          "[PlanetTransition] Safety timeout triggered! Forcing completion",
        );
  ...
  useFrame((state, dt) => {
    ...
    const altitudeFactor =
      direction === "takeoff" ? 1 - normalizedAltitude : normalizedAltitude;
    const shake =
      0.04 * (1 - Math.cos(thrustT * Math.PI)) * altitudeFactor * (1 - u);
    ...
    if (direction === "takeoff") {
      ...
      const starT = clamp01(
        (t - starFadeStart) / (starFadeEnd - starFadeStart),
      );
      ...
      const starT =
        1.0 -
        clamp01((t - starFadeOutStart) / (starFadeOutEnd - starFadeOutStart));
      setStarfieldOpacity(starT);
    }
    ...
    <>
      ...
      {direction === "takeoff" && (
        <SmokeTrail
          ...
          emit={
            clockRef.current / duration > 0.12 &&
            clockRef.current / duration < 0.7
          }
        />
      )}
      ...
      {direction === "takeoff" && (
        <EngineGlow intensity={engineGlowIntensity} />
      )}
