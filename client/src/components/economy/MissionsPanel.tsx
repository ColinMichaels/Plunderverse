...
export function MissionsPanel() {
  const { missions: legacyMissions, bounties, completedMissions, completedBounties, generateNewMissions } = useMissions();
  ...
  const [activeTab, setActiveTab] = useState<'plunderverse' | 'legacy' | 'reputation' | 'stats'>('plunderverse');
  ...
  const renderObjective = (objective: MissionObjective, missionId: string) => {
    const progress = plunderverseMissions.currentObjectiveProgress.get(missionId)?.get(objective.id) || 0;
    ...
    return (
      <div key={objective.id} className="flex items-center space-x-2 text-xs">
        ...
        <span className={`flex-1 ${objective.completed ? 'line-through text-gray-500' : 'text-gray-300'}`}>
        ...
        <div className={`flex items-start justify-between mb-2 ${!availability.available ? 'mt-6' : ''}`}>
          ...
          <div className="flex items-center space-x-1">
            ...
            {mission.rewards?.base?.credits && (
              <span className="text-yellow-400 font-mono text-sm">+{mission.rewards.base.credits}</span>
            )}
          </div>
        </div>
        
        <p className="text-gray-300 text-sm mb-2 italic">{mission.description}</p>
        ...
        {mission.requirements && Object.keys(mission.requirements).length > 0 && (
          <div className="text-xs text-gray-400 mb-2">
            ...
            {mission.requirements.reputation && (
              <span className="ml-2">
                Rep: {Object.entries(mission.requirements.reputation).map(([f, v]) => `${f}: ${v}`).join(', ')}
              </span>
            )}
    ...
    <>
      <div className="space-y-3">
        ...
        <div className="missions-panel-header bg-gray-800 p-4 border-b border-gray-600 rounded-t-lg">
          ...
          <div className="grid grid-cols-4 gap-2 text-xs">
            <div className="text-center">
              <div className="text-yellow-400 font-mono text-lg">💰 {credits}</div>
              <div className="text-gray-400">Credits</div>
            </div>
            <div className="text-center">
              <div className={`font-mono text-lg ${player.notoriety > 50 ? 'text-purple-400' : 'text-gray-400'}`}>
                😈 {player.notoriety}
              </div>
              <div className="text-gray-400">Notoriety</div>
            </div>
            <div className="text-center">
              <div className={`font-mono text-lg ${player.heat > 50 ? 'text-red-400' : player.heat > 20 ? 'text-orange-400' : 'text-green-400'}`}>
                🚨 {player.heat}
              </div>
              <div className="text-gray-400">Heat</div>
            </div>
            <div className="text-center">
              <div className="text-green-400 font-mono text-lg">📈 {totalEarnings}</div>
        ...
        <div className="flex border-b border-gray-600">
          ...
          >
            Missions ({plunderverseMissions.availableMissions.length + plunderverseMissions.activeMissions.length})
          </button>
          <button
        ...
        <div className="max-h-96 overflow-y-auto p-4 space-y-3">
          {activeTab === 'plunderverse' && (
            <>
              ...
              {plunderverseMissions.activeMissions.length > 0 && (
                <>
                  <h3 className="text-sm font-bold text-white mb-2">Active Missions</h3>
                  {plunderverseMissions.activeMissions.map(mission => renderMissionCard(mission, true))}
                  <hr className="border-gray-600 my-3" />
                </>
              )}
              
              {/* Available Missions */}
              <h3 className="text-sm font-bold text-white mb-2">Available Missions</h3>
              {plunderverseMissions.availableMissions.length === 0 ? (
                <div className="text-center py-8">
                  <div className="text-gray-400 mb-4">No missions available at this location</div>
                  <button
                    onClick={() => {
          ...
          {activeTab === 'legacy' && (
            <>
              <h3 className="text-sm font-bold text-white mb-2">Legacy Missions</h3>
              {legacyMissions.length === 0 ? (
                <div className="text-center py-8">
                  <div className="text-gray-400 mb-4">No legacy missions</div>
                  <button
          ...
          {activeTab === 'reputation' && (
            <div className="space-y-4">
                  ...
                  <div className="w-full bg-gray-700 rounded-full h-2">
                    <div
                      ...
                      style={{ 
                        ...
                        marginLeft: player.reputation.corporations < 0 ? `${50 - Math.abs(player.reputation.corporations / 2)}%` : '50%'
                      }}
                    />
                  </div>
                  ...
                  <div className="w-full bg-gray-700 rounded-full h-2">
                    <div
                      ...
                      style={{ 
                        ...
                        marginLeft: player.reputation.independents < 0 ? `${50 - Math.abs(player.reputation.independents / 2)}%` : '50%'
                      }}
                    />
                  </div>
                  <p className="text-xs text-gray-400 mt-1">
                  ...
                  <div className="w-full bg-gray-700 rounded-full h-2">
                    <div
                      ...
                      style={{ 
                        ...
                        marginLeft: player.reputation.outlaws < 0 ? `${50 - Math.abs(player.reputation.outlaws / 2)}%` : '50%'
                      }}
                    />
                  </div>
                  <p className="text-xs text-gray-400 mt-1">
              ...
              {/* Reputation Effects */}
              <div className="bg-gray-800 p-3 rounded border border-gray-600">
                <h3 className="font-semibold text-white mb-2 text-sm">Reputation Effects</h3>
                <div className="space-y-1 text-xs text-gray-400">
                  <div>• High corporate rep: Better prices at stations, access to restricted areas</div>
              ...
              <div className="bg-gray-800 p-3 rounded border border-gray-600">
                ...
                <div className="space-y-2 text-sm">
                  <div className="flex justify-between">
                    ...
                    <span className="text-purple-400">{player.rankTitle} (Level {player.rank})</span>
                  </div>
                  <div className="flex justify-between">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className={player.heat > 50 ? 'text-red-400' : 'text-orange-400'}>{player.heat}/100</span>
                  </div>
                  <div className="flex justify-between">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="text-purple-400">{plunderverseMissions.completedMissionIds.size + completedMissions.length}</span>
                  </div>
                  <div className="flex justify-between">
      ...
      {selectedMission && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50" onClick={() => setSelectedMission(null)}>
          <div className="bg-gray-900 border border-gray-600 rounded-lg p-6 max-w-2xl w-full mx-4 max-h-[80vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
              ...
              <div>
                ...
                <div className="flex items-center space-x-3">
                  ...
                  {selectedMission.rewards?.base?.credits && (
                    <span className="text-yellow-400 font-mono">💰 {selectedMission.rewards.base.credits} credits</span>
                  )}
                </div>
              </div>
              <button
            ...
            <p className="text-gray-300 mb-4 italic">{selectedMission.description}</p>
                  ...
                  {selectedMission.objectives.map(obj => (
                    <div key={obj.id} className="flex items-start space-x-2">
                      ...
                      <div>
                        ...
                        {obj.dialogue && (
                          <p className="text-xs text-gray-500 italic mt-1">"{obj.dialogue.npc}"</p>
                        )}
                      </div>
                    </div>
                  ))}
            ...
            {selectedMission.choices && selectedMission.choices.length > 0 && (
              <div className="mb-4">
                <h3 className="text-sm font-bold text-purple-400 mb-2">💭 This mission involves moral choices</h3>
                <p className="text-xs text-gray-400">Your decisions will impact faction relationships and future opportunities</p>
              </div>
            )}
            ...
            {selectedMission.requirements && Object.keys(selectedMission.requirements).length > 0 && (
              <div className="mb-4">
                ...
                <div className="text-sm text-gray-400">
                  ...
                  {selectedMission.requirements.reputation && Object.entries(selectedMission.requirements.reputation).map(([faction, req]) => (
                    <div key={faction}>• {faction} reputation: {req}</div>
                  ))}
            ...
            <div className="mb-4">
              <h3 className="text-sm font-bold text-white mb-2">Consequences:</h3>
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div>
                  <h4 className="text-green-400 mb-1">Success:</h4>
                  {selectedMission.rewards?.base?.reputation && (
                    <div className="text-gray-400">
                      {Object.entries(selectedMission.rewards.base.reputation).map(([faction, change]) => (
                        <div key={faction} className={change > 0 ? 'text-green-400' : 'text-red-400'}>
                          {faction}: {change > 0 ? '+' : ''}{change}
                        </div>
                      ))}
                    </div>
                ...
                {selectedMission.failureConsequences && (
                  <div>
                    ...
                    <div className="text-gray-400">
                      ...
                      {selectedMission.failureConsequences.reputation && Object.entries(selectedMission.failureConsequences.reputation).map(([faction, change]) => (
                        <div key={faction} className="text-red-400">{faction}: {change}</div>
                      ))}
                    </div>
                  </div>
                )}
      ...
      {showChoiceDialog && selectedMission && selectedMission.choices.length > 0 && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50">
          <div className="bg-gray-900 border border-purple-600 rounded-lg p-6 max-w-2xl w-full mx-4">
            <h2 className="text-xl font-bold text-white mb-4">Critical Decision</h2>
            <p className="text-gray-300 mb-4 italic">Choose carefully - your decision will have lasting consequences...</p>
            ...
            <div className="space-y-3">
              {selectedMission.choices.map(choice => (
                <button
                  ...
                  className="w-full text-left p-4 bg-gray-800 hover:bg-gray-700 border border-gray-600 hover:border-purple-500 rounded transition-all"
                >
