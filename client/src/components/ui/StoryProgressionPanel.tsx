...
export const StoryProgressionPanel: React.FC<StoryProgressionPanelProps> = ({
  ...
  useEffect(() => {
    ...
    const updateStoryState = async () => {
      try {
        ...
        console.log(
          "[StoryProgressionPanel] Getting story progression state...",
        );
        const state = gameFacade.getStoryProgressionState();
        console.log(
          "[StoryProgressionPanel] Story progression state received:",
          state,
        );

        if (!state) {
          console.warn(
            "[StoryProgressionPanel] No story state returned from gameFacade",
          );
        ...
        console.log(
          "[StoryProgressionPanel] Getting current act info for act:",
          state.currentAct,
        );
        try {
          const act = await gameFacade.getCurrentAct();
          console.log(
            "[StoryProgressionPanel] Current act info received:",
            act,
          );

          if (!act) {
            console.warn(
              "[StoryProgressionPanel] No act data returned for act:",
              state.currentAct,
            );
          ...
          console.error(
            "[StoryProgressionPanel] Error getting current act:",
        ...
        try {
          ...
          console.log(
            "[StoryProgressionPanel] Rank progress received:",
            progress,
          );
          setRankProgress(progress);
        } catch (progressError) {
          console.error(
            "[StoryProgressionPanel] Error getting rank progression:",
        ...
        if (state.currentAct === 4) {
          console.log(
            "[StoryProgressionPanel] Act 4 detected, getting available endings...",
          );
          try {
            const endings = await gameFacade.getAvailableEndings();
            console.log(
              "[StoryProgressionPanel] Available endings received:",
              endings,
            );
            setAvailableEndings(endings || []);
          } catch (endingsError) {
            console.error(
              "[StoryProgressionPanel] Error getting available endings:",
              endingsError instanceof Error
                ? endingsError.message
                : String(endingsError),
            );
            setAvailableEndings([]);
          }
        ...
        console.error(
          "[StoryProgressionPanel] Fatal error updating story state:",
          error instanceof Error ? error.message : String(error),
        );
        setError(
          error instanceof Error
            ? error.message
            : "Failed to load story progression",
        );
        setIsLoading(false);
    ...
    return () => {
      console.log(
        "[StoryProgressionPanel] Component unmounting, clearing interval",
      );
      ...
      <Card className={`${className} bg-gray-900/95 border-gray-700`}>
        <CardContent className="p-4">
          <div className="text-gray-400">
            Story progression data not available
          </div>
        </CardContent>
      </Card>
    );
  ...
  const getMoralityIcon = () => {
    if (!storyState?.moralityScore)
      return <Shield className="w-4 h-4 text-gray-400" />;
    if (storyState.moralityScore >= 50)
          ...
          <TabsContent value="overview" className="space-y-4 mt-4">
            ...
            <div className="bg-gray-800 rounded-lg p-4 space-y-3">
              <h3 className="text-lg font-semibold text-purple-400">
                {currentAct.title}
                {currentAct.subtitle ? `: ${currentAct.subtitle}` : ""}
              </h3>
              <p className="text-sm text-gray-300">{currentAct.description}</p>

              <div className="flex items-center gap-4 text-sm">
                <span className="text-gray-400">
                  Rank Range: {currentAct.rankRange?.min || 0} -{" "}
                  {currentAct.rankRange?.max || 10}
                </span>
                <span className="text-gray-400">
            ...
            <div className="bg-gray-800 rounded-lg p-4 space-y-3">
                  ...
                  <div
                    ...
                    style={{
                      left:
                        (storyState.moralityScore || 0) >= 0
                          ? "50%"
                          : `${50 + (storyState.moralityScore || 0) / 2}%`,
              ...
              <div className="text-xs text-gray-400">
                Score: {(storyState.moralityScore || 0) > 0 ? "+" : ""}
                {storyState.moralityScore || 0}
              </div>
            </div>
                  ...
                  <div className="space-y-2">
                    <div className="flex justify-between text-xs">
                      ...
                      <span className="text-yellow-400">
                        {(rankProgress.progress?.credits || 0).toLocaleString()}{" "}
                        /{" "}
                        {rankProgress.nextRank
                          ? rankProgress.nextRank === 1
                            ? "5,000"
                            : rankProgress.nextRank === 2
                              ? "15,000"
                              : rankProgress.nextRank === 3
                                ? "35,000"
                                : rankProgress.nextRank === 4
                                  ? "60,000"
                                  : rankProgress.nextRank === 5
                                    ? "100,000"
                                    : rankProgress.nextRank === 6
                                      ? "200,000"
                                      : rankProgress.nextRank === 7
                                        ? "350,000"
                                        : rankProgress.nextRank === 8
                                          ? "500,000"
                                          : rankProgress.nextRank === 9
                                            ? "750,000"
                                            : "1,000,000"
                          : "MAX"}
                      </span>
                    ...
                    <Progress
                      value={Math.min(
                        100,
                        ((rankProgress.progress?.credits || 0) /
                          (rankProgress.nextRank === 1
                            ? 5000
                            : rankProgress.nextRank === 2
                              ? 15000
                  ...
                  <div className="space-y-2">
                    <div className="flex justify-between text-xs">
                      ...
                      <span className="text-blue-400">
                        {rankProgress.progress?.missions || 0} /{" "}
                        {rankProgress.nextRank
                          ? rankProgress.nextRank === 1
                            ? "3"
                            : rankProgress.nextRank === 2
                  ...
                  <div className="space-y-2">
                    <div className="flex justify-between text-xs">
                      ...
                      <span className="text-red-400">
                        {rankProgress.progress?.notoriety || 0} /{" "}
                        {rankProgress.nextRank
                          ? rankProgress.nextRank === 1
                            ? "10"
                            : rankProgress.nextRank === 2
            ...
            <div className="bg-gray-800 rounded-lg p-4 space-y-3">
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="text-xs text-gray-400">
                  {storyState.completedMissions || 0} /{" "}
                  {storyState.totalStoryMissions || 0} Complete
                </span>
              </div>
              <Progress
                value={
                  storyState.totalStoryMissions > 0
                    ? ((storyState.completedMissions || 0) /
                        storyState.totalStoryMissions) *
                      100
                    : 0
                }
                className="h-2"
            ...
            <ScrollArea className="h-[400px]">
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="bg-gray-800 rounded-lg p-3">
                    ...
                    <div className="text-lg font-bold text-yellow-400">
                      {(
                        storyState.progressionMetrics?.creditsEarnedTotal || 0
                      ).toLocaleString()}
                    </div>
                  </div>
                  ...
                  <div className="bg-gray-800 rounded-lg p-3">
                    ...
                    <div className="text-lg font-bold text-purple-400">
                      {storyState.progressionMetrics?.systemsVisited?.size ||
                        storyState.progressionMetrics?.systemsVisited?.length ||
                        0}
                    </div>
                  </div>
                ...
                {storyState.progressionMetrics?.missionsByType &&
                  Object.keys(storyState.progressionMetrics.missionsByType)
                    .length > 0 && (
