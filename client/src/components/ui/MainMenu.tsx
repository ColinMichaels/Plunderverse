...
export const MainMenu: React.FC = () => {
  ...
  const [activePanel, setActivePanel] = useState<"main" | "saves" | "settings">(
    "main",
  );
  ...
  const handleLoadSave = async (slot: number) => {
    ...
    try {
      ...
      const hasConflict = await cloudSyncManager.checkForConflict(
        gameState.timestamp,
      );
      ...
      if (hasConflict) {
        ...
        setError(
          "This save conflicts with your cloud save. Check the sync status to resolve.",
        );
        setIsLoading(false);
        return;
      }
  ...
  if (!isOpen) {
    return (
      <button
        ...
        className="fixed top-24 right-4 p-2 bg-slate-900/80 border border-orange-600/30 rounded-lg text-orange-400 hover:bg-slate-800 transition-all z-40"
        title="Menu (ESC)"
      >
              ...
              <div className="space-y-3">
                {isLoading ? (
                  ...
                  <>
                    ...
                    <button
                      onClick={() =>
                        handleSaveGame(saves[0]?.slot || availableSlots[0] || 1)
                      }
                      disabled={isLoading}
