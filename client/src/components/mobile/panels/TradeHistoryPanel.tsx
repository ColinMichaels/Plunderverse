...
export const TradeHistoryPanel: React.FC<TradeHistoryPanelProps> = ({ onClose }) => {
  ...
  return (
    <div className="flex flex-col h-full bg-black">
      {/* Header */}
      <header className="bg-gradient-to-b from-slate-900 to-slate-800 border-b border-purple-600/30 p-2">
        <div className="flex items-center justify-between mb-1">
          ...
          <button
            ...
            className="p-1 rounded bg-slate-700/50 text-red-400"
            title="Clear history"
          >
          ...
          {(['all', 'buy', 'sell', 'profitable', 'losses'] as FilterType[]).map(type => (
            <button
              ...
              className={`px-2 py-0.5 rounded text-[10px] font-semibold whitespace-nowrap transition-colors
                       ${filterType === type ? 'bg-orange-600 text-white' : 'bg-slate-600 text-gray-300'}`}
            >
