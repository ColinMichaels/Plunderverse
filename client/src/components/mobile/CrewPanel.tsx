...
function SectionTitle({children}: { children: React.ReactNode }) {
    return (
        <h3 className="text-xs font-semibold text-white mb-2 tracking-wide">{children}</h3>
    );
}
...
function TabButton({
    ...
    return (
        <button
            onClick={onClick}
            className={`flex-1 py-1.5 px-2 text-xs font-medium transition-all ${
                active
...
const getCrewEfficiency = (member: any) => {
    const avgSkill =
        Object.values(member.skills).reduce((sum: number, val) => sum + (val as number), 0) /
...
export const CrewPanel: React.FC<CrewPanelProps> = ({ onClose }) => {
    ...
    const handleAssignTask = (crewId: string, taskId: string) => {
        ...
        if (task) {
            const result = crew.assignTask(crewId, taskId, task.name, task.duration, task.skill);
            if (result.success) {
                toast.success(result.message, {
    ...
    return (
        <div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center p-4">
            <div
                className="bg-gray-900 border border-cyan-400/50 rounded-lg w-full max-w-2xl max-h-[80vh] overflow-hidden flex flex-col">
                {/* Header */}
                <div
                    className="flex items-center justify-between p-2 border-b border-cyan-400/30 bg-gradient-to-r from-gray-900 to-gray-800">
                ...
                <div className="flex border-b border-cyan-400/30 bg-gray-800/30">
                    ...
                    <TabButton active={selectedTab === 'available'} onClick={() => setSelectedTab('available')}>
                    ...
                    {selectedTab === 'active' ? (
                        crew.activeCrew.length === 0 ? (
                            ...
                            crew.activeCrew.map((member) => {
                                ...
                                const primarySkill = Object.entries(member.skills).sort(
                                ...
                                return (
                                    ...
                                    >
                                        {/* Crew Header */}
                                        <div className="p-3 cursor-pointer" onClick={() => toggleExpanded(member.id)}>
                                            <div className="flex items-start gap-3">
                                                {/* Portrait */}
                                                <div
                                                    className="w-14 h-14 bg-gradient-to-br from-cyan-600 to-cyan-800 rounded-lg flex items-center justify-center text-2xl flex-shrink-0">
                                                    {portrait}
                                                </div>
                                                ...
                                                <div className="flex-1 min-w-0">
                                                    <div className="flex items-start justify-between mb-1">
                                                        <div>
                                                            <h3 className="text-cyan-400 font-bold text-base">{member.name}</h3>
                                                    ...
                                                    <div className="grid grid-cols-3 gap-2 mt-3">
                                                        <div className="bg-gray-900/50 rounded p-2">
                                                            <div className="flex items-center gap-1 mb-1">
                                                                <StatusIcon
                                                                    className={`w-3 h-3 ${loyaltyStatus.color}`}/>
                                                        ...
                                                        <div className="bg-gray-900/50 rounded p-2">
                                                            <div className="flex items-center gap-1 mb-1">
                                                                ...
                                                                <span
                                                                    className="text-xs text-gray-400">Efficiency</span>
                                                            </div>
                                                            <div
                                                                className="text-sm font-bold text-yellow-400">{efficiency}%
                                                            </div>
                                                        </div>
                                                        ...
                                                        <div className="bg-gray-900/50 rounded p-2">
                                                            ...
                                                            <div
                                                                className="text-sm font-bold text-white">{member.dailySalary}c
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
                                        ...
                                        {/* Expanded Details */}
                                        {isExpanded && (
                                            <div className="border-t border-cyan-400/20 p-3 space-y-3 bg-gray-900/30">
                                                {/* Skills Breakdown */}
                                                <div>
                                                        ...
                                                        {Object.entries(member.skills).map(([skill, value]) => (
                                                            <div key={skill}>
                                                                <div className="flex justify-between text-xs mb-1">
                                                                    <span
                                                                        className="text-gray-400 capitalize">{skill}</span>
                                                                    <span
                                                                        className="text-white font-medium">{value}%</span>
                                                                </div>
                                                                <div
                                                ...
                                                <div>
                                                    ...
                                                    <div className="flex flex-wrap gap-1">
                                                        {member.personality.traits.map((trait: string) => (
                                                            <span
                                                                key={trait}
                                                                className="text-xs bg-purple-400/20 text-purple-400 px-2 py-1 rounded"
                                                            >
                                {trait}
                              </span>
                                                        ))}
                                    ...
                                    {member.currentTask.completed
                                        ? 'Completed!'
                                        : getTimeRemaining(member.currentTask)}
                                  </span>
                                                                </div>
                                      ...
                                      {Math.round(member.currentTask.progress)}%
                                    </span>
                                                                    </div>
                                                                    <div
                                                                ...
                                                                {AVAILABLE_TASKS.map((task) => {
                                                                    ...
                                                                    return (
                                                                        <button
                                                                            ...
                                                                            className="p-2.5 rounded-lg border bg-gray-700/50 border-gray-600 hover:bg-gray-600 text-gray-300 hover:border-cyan-400 transition-all"
                                                                            title={`${task.name} (${task.displayDuration})\nSkill: ${skillValue}%`}
                                                                        >
                                                                            <TaskIcon className="w-5 h-5 mx-auto mb-1"/>
                                                                            <div
                                                                                className="text-xs font-medium">{task.name.split(' ')[0]}</div>
                                                                            <div
                                                                                className="text-xs text-gray-400 mt-1">{skillValue}%
                                                                            </div>
                                                                        </button>
                                                                    );
                                                                })}
                                    ...
                                    >
                                        <div className="p-3 cursor-pointer" onClick={() => toggleExpanded(member.id)}>
                                            <div className="flex items-start gap-3">
                                                {/* Portrait */}
                                                <div
                                                    className="w-14 h-14 bg-gradient-to-br from-gray-600 to-gray-800 rounded-lg flex items-center justify-center text-2xl flex-shrink-0">
                                                    {portrait}
                                                </div>
                                                ...
                                                <div className="flex-1 min-w-0">
                                                    ...
                                                    {!isExpanded && (
                                                        <p className="text-gray-400 text-xs line-clamp-2">{member.background}</p>
                                                    )}
                                                </div>
                                            </div>
                                        </div>
                                        ...
                                        {isExpanded && (
                                            <div className="border-t border-cyan-400/20 p-3 space-y-3 bg-gray-900/30">
                                                        ...
                                                        {Object.entries(member.skills).map(([skill, value]) => (
                                                            <div key={skill} className="flex justify-between text-xs">
                                                                <span
                                                                    className="text-gray-400 capitalize">{skill}</span>
                                                                <span
                                                                    className="text-cyan-400 font-medium">{value}%</span>
                                                            </div>
                                                        ))}
                                                    </div>
                                                </div>
                                                ...
                                                {/* Hiring Info */}
                                                <div
                                                    className="pt-3 border-t border-cyan-400/20 grid grid-cols-2 gap-3 text-xs">
                                                    <div>
                                                        <span className="text-gray-400">Hiring Cost:</span>
                                                        <span
                                                            className="text-white ml-2 font-semibold">{member.hiringCost} cr</span>
                                                    </div>
                                                    <div>
                                                        <span className="text-gray-400">Daily Salary:</span>
                                                        <span
                                                            className="text-white ml-2 font-semibold">{member.dailySalary} cr/day</span>
                                                    </div>
                                                </div>
                                            </div>
                                        )}
                ...
                {/* Footer Stats */}
                <div className="border-t border-cyan-400/30 p-3 bg-gray-800/50">
                    <div className="flex justify-between text-xs">
                        <div>
                ...
                {crew.activeCrew.length}/{crew.maxCrewSize}
              </span>
                        </div>
                    </div>
                </div>
