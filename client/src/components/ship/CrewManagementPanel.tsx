...
import {
  useCrewManagement,
  CrewMember,
} from "../../lib/stores/ship/useCrewManagement";
...
export function CrewManagementPanel({
  ...
  return (
    <div className="max-w-7xl mx-auto p-4 bg-black/90 rounded-lg backdrop-blur-md">
      <div className="flex justify-between items-center mb-6 h-full max-h-[80vh]">
        <div>
          ...
          <p className="text-gray-400 mt-1">
            {activeCrew.length}/{maxCrewSize} crew members • Daily wages:{" "}
            {dailySalaryCosts} credits
          </p>
        </div>
      ...
      {paymentMessage && (
        <Alert
          className="mb-4"
          variant={
            paymentMessage.includes("Cannot") ? "destructive" : "default"
          }
        >
              ...
              {activeCrew.length === 0 ? (
                ...
                activeCrew.map((crew) => {
                  ...
                  const topSkill = getTopSkill(
                    crew.skills as unknown as Record<string, number>,
                  );
                      ...
                      <CardContent className="space-y-3">
                        <div className="space-y-2">
                          <div className="flex justify-between text-sm">
                            ...
                            <span className={loyaltyStatus.color}>
                              {crew.currentLoyalty}%
                            </span>
                          </div>
                          <Progress
                            value={crew.currentLoyalty}
                            className="h-2"
                          />
                        </div>
                        ...
                        {crew.personalQuest?.active && (
                          <Badge
                            variant="default"
                            className="text-xs bg-yellow-900"
                          >
                        ...
                        <CardContent className="space-y-4">
                          <div>
                            ...
                            <p className="text-sm text-gray-300 italic">
                              "
                              {crew.currentLoyalty >= 60
                                ? crew.dialogue.highMorale
                                : crew.dialogue.lowMorale}
                              "
                            </p>
                          </div>
                            ...
                            <div>
                              ...
                              <div className="space-y-2">
                                <p className="text-sm text-yellow-400">
                                  {crew.personalQuest.title}
                                </p>
                                {!crew.personalQuest.active &&
                                  !crew.personalQuest.completed && (
                                    <p className="text-xs text-gray-400">
                                      Requires{" "}
                                      {crew.personalQuest.triggerLoyalty}{" "}
                                      loyalty (current: {crew.currentLoyalty})
                                    </p>
                                  )}
                                {crew.personalQuest.active && (
                                  <div className="space-y-1">
                                    <p className="text-xs text-gray-400">
                                      Stage{" "}
                                      {(crew.personalQuest.currentStage || 0) +
                                        1}{" "}
                                      of {crew.personalQuest.stages.length}
                                    </p>
                                    <p className="text-xs text-blue-400">
                                      {
                                        crew.personalQuest.stages[
                                          crew.personalQuest.currentStage || 0
                                        ]
                                      }
                                    </p>
                                ...
                                {crew.personalQuest.completed && (
                                  <Badge
                                    variant="default"
                                    className="text-xs bg-green-900"
                                  >
                                    <Award className="w-3 h-3 mr-1" />
                                    Quest Completed
                                  </Badge>
            ...
            <CardContent>
              ...
              {Object.values(currentBonuses).every((v) => v === 0) && (
                <p className="text-center text-gray-500 py-4">
                  No active bonuses. Crew need higher loyalty levels to provide
                  bonuses.
                </p>
              )}
            </CardContent>
          ...
          <Card>
            ...
            <CardContent>
              <div className="space-y-3">
                {activeCrew
                  ...
                  .map((crew) => (
                    <div
                      key={crew.id}
                      className="flex items-center justify-between p-3 bg-purple-900/20 rounded"
                    >
                      <div>
              ...
              {!activeCrew.some((c) => c.currentLoyalty >= 95) && (
                <p className="text-center text-gray-500 py-4">
                  No special abilities unlocked yet. Reach 95+ loyalty with crew
                  members.
                </p>
              )}
            </CardContent>
              ...
              <div className="space-y-2 max-h-[400px] overflow-y-auto">
                {crewEvents
                  ...
                  .map((event) => {
                    const crew = [
                      ...activeCrew,
                      ...useCrewManagement.getState().availableCrew,
