...
export function MissionsPanel({ isVisible, onToggle }: MissionsPanelProps) {
  ...
  if (!isVisible) {
    return (
      <button
        onClick={onToggle}
        className="fixed right-4 top-20 bg-blue-600 hover:bg-blue-500 text-white p-3 rounded-lg border border-blue-400 transition-colors z-40"
        title="Open Missions Panel"
      >
        📋 Missions
      </button>
    );
