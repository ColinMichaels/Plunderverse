...
export const useFlashlight = create<FlashlightState>((set, get) => ({
  ...
  isCharging: false,
  maxBattery: 100,
  drainRate: 2, // Drain 2% per second when on (50 seconds of continuous use)
  ...
  toggle: () => {
    ...
    if (state.isOn) {
      ...
      set({ isOn: true });
      console.log(`[FLASHLIGHT] Turned on (Battery: ${Math.round(state.batteryLevel)}%)`);
    } else {
  ...
  updateBattery: (deltaTime: number) => {
    ...
    if (newBatteryLevel !== state.batteryLevel) {
      set({ batteryLevel: Math.max(0, Math.min(state.maxBattery, newBatteryLevel)) });
    }
  },
