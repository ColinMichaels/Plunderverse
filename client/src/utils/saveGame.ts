...
export function collectGameState(): GameStateData {
    ...
    const gameState: GameStateData = {
        ...
        stores: {
            ...
            tradeHistory: {
                history: tradeHistory.transactions,
                analytics: {
                    totalTrades: 0,
                    totalProfit: 0,
                    totalLoss: 0,
            ...
            heat: {
                ...
                lastActionTimestamp: heat.lastEncounterTime ?? Date.now(),
                decayRate: heat.getHeatDecayRate() ?? 0.1,
            },

            rewards: {
                landingRewards: rewards.totalEarnings,
...
// Restore game state to all stores. Adjusted to remove console logs.
export function restoreGameState(gameState: GameStateData | any): void {
    ...
    if (stores.inventory) {
        ...
        inventoryState.storageCapacity = stores.inventory.capacity ?? 100;
    }
    ...
    if (stores.crew) {
        ...
        crewState.activeCrew = stores.crew.crewMembers ?? [];
        crewState.maxCrewSize = stores.crew.maxCrewSize ?? 4;
        crewState.dailySalaryCosts = stores.crew.totalWages ?? 0;
    }

    // Plunderverse economy
    ...
    if (stores.tradeHistory) {
        ...
        tradeState.transactions = stores.tradeHistory.history ?? [];
    }
    ...
    if (stores.landedState) {
        const landedStateRef = useLandedState.getState();
        if (stores.landedState.isLanded !== undefined) {
            landedStateRef.isLanded = stores.landedState.isLanded;
        }
    }
    ...
    // Return to playing phase after restore
